...
<?php if ($this->Session->read('categoria') === 'administrador'): ?>
    ...
    <?php echo $this->Form->input('limite', array('type' => 'select', 'label' => array('text' => 'Registros', 'style' => 'display: inline'), 'options' => array(0 => "0", 10 => "10", 20 => "20", 50 => "50"), 'default' => $limite)); ?>
    <?php echo $this->Form->end(); ?>
...
<table>
    <thead>
        <tr>
            ...
            <th>
                <?php echo $this->Paginator->sort('Instituicao.virtualMaxData', 'Visita'); ?>
            </th>            
            <th>                
    ...
    <tbody>
        <?php foreach ($instituicoes as $c_instituicao): ?>
            ...
            <tr>
                ...
                <td>
                    <?php
                    if (!empty($c_instituicao['Visita'])) {
                        foreach ($c_instituicao['Visita'] as $c_visita) {
                            // pr($c_visita);
                            echo $this->Html->link(date('d-m-Y', strtotime($c_visita['data'])), '/instituicaos/view/' . $c_visita['estagio_id']);
                        }
                    }
                    ?>
                </td>
                <td><?php echo $c_instituicao['Estagiario']['periodo'];   ?></td>
                <td><?php // echo $c_instituicao['Estagiario']['estudantes'];   ?></td>
                <td><?php // echo $c_instituicao['Estagiario']['supervisores'];   ?></td>
                <td><?php // echo $c_instituicao['Instituicao']['area'];   ?></td>
