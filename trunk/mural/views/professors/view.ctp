...
<tr>
<td width='25%'>Nome</td>
<td width='75%'><?php echo $html->link($professor['Professor']['nome'], '/Estagiarios/index/id_professor:' . $professor['Professor']['id']); ?></td>
</tr>
...
<td>
	...
	if ($professor['Professor']['curriculolattes']) {
		echo $html->link('Lattes', $professor['Professor']['curriculolattes']); 
	} else {
		echo "Sem dados";
	}
	?>
...
<?php 
	if ($professor['Professor']['curriculosigma']) {
		echo $html->link('Sigma','http://www.sigma-foco.scire.coppe.ufrj.br/UFRJ/SIGMA_FOCO/REMOTO/cadastros.htm?codigo='.$professor['Professor']['curriculosigma']); 
	} else {
		echo "Sem Sigma";
	}
?>
...
<td>
	...
	if ($professor['Professor']['pesquisadordgp']) {
		echo $html->link('Pesquisador', 'http://dgp.cnpq.br/buscaoperacional/detalhepesq.jsp?pesq='.$professor['Professor']['pesquisadordgp']);
	} else {
		echo "Sem dados";
	}
	?>
...
echo " | ";
echo $html->link('Editar', '/Professors/edit/' . $professor['Professor']['id']);
echo " | ";
echo $html->link('Inserir', '/Professors/add/');
echo " | ";
echo $html->link('Listar', '/Professors/index/');
